...
Gibbs_Sampler_Individual <- function(dataout, chain_length, burn_in_rate, k, t_1, t_2 ,narm = 3, adjustment.method = NULL){
  ...
  estimated_ci = c(quantile(simulation_mu[store_row:chain_length],0.025),
                   quantile(simulation_mu[store_row:chain_length],0.975))
  ...
  result = list(estimated_mu, estimated_mu_var,estimated_ci,
  ...
  Gibbs_Sampler_Overall <- function(dataout, chain_length, burn_in_rate, adjustment.method = NULL, narm = 3){
    ...
    estimated_ci = matrix(0, nrow = narm*2, ncol=2)
    for(k in 1:2){
        for(t1 in 1:(narm-1)){
          for(t2 in (t1+1):narm){
            start_time <- Sys.time()
            ...
            estimated_ci[(k-1)*3+tau_index,1:2] = sim_result[[3]]
            J_mu[(k-1)*3+tau_index] =  sim_result[[4]]
            diag_H_mu[(k-1)*3+tau_index] = sim_result[[5]]  
   ...
   return(list(estimated_mu,estimated_ci,variance_mu_mcmc, variance_mu_sandwich))
  }
