---
title: "Adjusted Gibbs Sampling Algorithm"
...
& =\frac{L\left(\tau_k^{XZ^2}, \mu_k^{XZ}\right)}{L\left(\tau_k^{XZ^2}\right)} \text{ Given } P\left(\tau_k^{XZ^2},\mu_k^{XZ}\right) \propto\left(\tau_k^{XZ^2}\right)^{-1} \\
...
Algorithm: Adjusted Gibbs Sampling algorithm
...
|               sample $\mu_k^{XZ(p)} \sim N(\bar{y}_{XZ}^k, \left(\sigma_k^{XZ}\right)^2)$ where $\left(\sigma_k^{XZ}\right)^2 = \frac{n_{xz}^k }{\sum_{i \in N_{XZ}^k} \frac{1}{\left(s_{k,i}^{XZ}+\tau_k^{XZ^2}\right)}}$ and $\bar{y}_{xz}^k = \frac{1}{n_{xz}^k} \sum_{i \in N_{XZ}^k} y_{k,i}^{xz}$
|     $\eta^{(t)} = \left(\tau^{(p)}, \mu^{(p)}\right)$
