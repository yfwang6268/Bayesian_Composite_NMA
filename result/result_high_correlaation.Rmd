...
mu_var_sand = colMeans(mu_var_sand)
mu_var_sim = apply(mu_posterior,2,var)
var_result_lc = rbind(mu_var_pos,mu_var_sand, mu_var_sim)
rownames(var_result_lc) = c("Posterior", "Sandwich", "Simulation")
colnames(var_result_lc) = c("BA1", "CA1","BC1", "BA2", "CA2", "BC2")
...
no_adjust_bias = abs(colMeans(mu_posterior) - mu)
netmeta_bias = abs(colMeans(netmeta_estimate) - mu)
gemtc_bias = abs(colMeans(gemtc_estimate) - mu)
bias_result_lc = rbind(no_adjust_bias,gemtc_bias , netmeta_bias)
colnames(bias_result_lc) = c("BA1", "CA1","BC1", "BA2", "CA2", "BC2")
rownames(bias_result_lc) = c("no adjust", "gemtc", "netmeta")
```

This file is to compare the bias and  variance from the simulation result when the correlation is 0.5. For the bias, we calculate the three variance:
...
```
When the between study correlation is 0.1 and the within study correlation is 0.2 (setting 1 in Duan2020), the bias result is
```{r, echo=FALSE}
knitr::kable(round(bias_result_lc,4))
```
...
```
When the between  study correlation is 0.1 and the within study correlation is 0.2 (setting 1 in Duan2020), the variance result is
```{r, echo=FALSE}

knitr::kable(round(var_result_lc,4))
```
