---
title: "MCMC_algorithm_derivation"
author: "Yifei Wang"
...
\end{aligned}
$$
Thus,by choosing $P\left(\tau_k^{XZ^2}, \mu^{XZ}\right) \propto \left(\tau_k^{XZ^2}\right)^{-1}$,  the posterior distribution of $\tau_k^{XZ^2}$ follows
$$
...
We use Metropolis-Hasting algorithm to sample $\tau_k^{XZ}$ from $p(\tau_k^{XZ^2}|y)$.
...
|     $a=\frac{P\left(\tau_{k,*}^{XZ^2} | y \right)/q\left(\tau_{k,*}^{XZ^2} | \tau_{k,t-1}^{XZ^2}\right)}{P\left(\tau_{k,t-1}^{XZ^2}\right)/q\left(\tau_{k,t-1}^{XZ^2} | \tau_{k,*}^{XZ^2},\right)}$
|     if $runif(1) < min(a,1)$:
