...
1. We reduce the length of markov chain due to the computation burden.
   ...
   * Gibbs Sampler: 5000 to 1000
   * MH algorithm: 500 to 100
   
   The markov chain is too short to reach the stable distribution.
...
Bias for 500 simulations

```{r, echo=FALSE}
no_adjust_bias = abs(apply(posterior_mu,2,mean) - mu)
...
colnames(bias_result) = trt_names
rownames(bias_result) = c("no adjust", "gemtc", "netmeta")
print(round(bias_result,3))
```

Variance for 500 simulations
